...
int main(int argc, char *argv[])
{
  ...
  if (argc < 2)
  {
    exitWithError("DEC_SERVER_USAGE: %s port\n", 1);
  }
  ...
  if (listenSocket < 0)
  {
    exitWithError("DEC_SERVER_ERROR opening socket", 1);
  }
  ...
  {
    exitWithError("DEC_SERVER_ERROR on binding", 1);
  }
  ...
  while (1)
  {
    ...
    if (charsRead < 0)
    {
      fprintf(stderr, "DEC_SERVER_ERROR writing to socket\n");
    }
